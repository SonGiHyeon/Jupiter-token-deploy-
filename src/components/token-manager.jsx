...
export function TokenManagerComponent() {
  ...
  useEffect(() => {
    if (isTxSuccess) {
      setMessage(`${transferAmount} 토큰이 성공적으로 ${recipientAddress} 전송되었습니다.`);
  ...
  const handleReceiveTokens = async () => {
    ...
    setTimeout(() => {
      setIsCooldown(false); // 일정 시간 후 쿨타운 해체
    }, 10000) // 하루(24시간 쿨다운) 60,000(1분), 360,000(1시간), 864,000(24시간)
    // } catch (error) {
    //   setMessage("토큰 받기 실패: " + error.message);
    //   setIsCooldown(false);
    // }
